...
<div class="col-sm-4 mb-5">
  ...
  if($result->num_rows > 0){
    while($row = $result->fetch_assoc()){
      ...
      echo '<h5 class="card-title">Request For : ' . $row['request_info'] . '</h5>';
      ...
      echo '<form action="" method="POST"> 
              ...
              <input type="button" class="btn btn-secondary" value="Close" onclick="confirmClose('.$row["request_id"].')">
            </form>';
      echo '</div>' ;
      echo '</div>' ;
      echo'</div>';
...
<script>
  ...
  function confirmClose(requestId) {
    if (confirm("Are you sure you want to close this request?")) {
      // Redirect to a PHP file with the request ID to handle deletion
      window.location.href = "?close&id=" + requestId;
    }
  }
</script>
